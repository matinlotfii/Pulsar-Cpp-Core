...
REMOTE_URL="ssh://${SYNC_REMOTE_USER}@${SYNC_REMOTE_HOST}:${SYNC_REMOTE_PORT}${SYNC_REMOTE_DIR}"

ssh "${SSH_OPTS[@]}" "$REMOTE" "mkdir -p '$SYNC_REMOTE_DIR'"
ssh "${SSH_OPTS[@]}" "$REMOTE" "
  if [ ! -d '$SYNC_REMOTE_DIR/.git' ]; then
    git init -b '$SYNC_GIT_BRANCH' '$SYNC_REMOTE_DIR'
  fi
  git -C '$SYNC_REMOTE_DIR' config receive.denyCurrentBranch updateInstead
"
...
if git -C "$ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  if git -C "$ROOT" remote get-url origin >/dev/null 2>&1; then
    ...
    if [[ "$current_origin" != "$REMOTE_URL" ]]; then
      log "Existing origin kept unchanged: $current_origin"
      log "Remote sync repo is ready at $REMOTE_URL"
      exit 0
    fi
  else
