...
#   1) Install missing runtime/build dependencies when possible.
#   2) Build the UI and optimized C++ core.
#   3) Run the mock/headless smoke test.
#   4) Create a verified Git checkpoint and local backups.
#   5) Push the branch and checkpoint tag atomically to GitHub over SSH port 443.
#   6) Sync to 192.168.1.123, build there, and restart pulsar-kiosk.service.
#
...
Default ./run.sh:
  Build UI + optimized C++ core
  -> run the mock/headless smoke test
  ...
  -> sync to 192.168.1.123, build, and restart pulsar-kiosk.service
